...
var deviceList = function(req,res,next){
  ...
  pool.acquire(function(err, connection){
	...
	var q = '"';
    var request = new Request("SELECT [id],[account] ,[userName],[orgName],[host_name],[mac],[os_name],[update_time],[match_time],'3' as limit FROM [AgileControllerDB].[dbo].[TSM_E_Account] a  join [AgileControllerDB].[dbo].[TSM_E_Endpoint] on [account] = [login_account]  join [AgileControllerDB].[dbo].[TSM_E_Organization] b on a.[orgID] = b.[orgID] where [login_account] = '"+req.param('accname')+"'", function(err, rowCount){

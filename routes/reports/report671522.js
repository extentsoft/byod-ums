...
var deviceList = function(req,res,next){
  ...
  pool.acquire(function(err, connection){
    ...
    var request = new Request("SELECT [os_name] OS,accountName Account, terminalMac MAC, timestamp 'Date' FROM [AgileControllerDB].[dbo].[TSM_E_RadiusLoginOrLogoutLog] a join [AgileControllerDB].[dbo].[TSM_E_Endpoint] b on a.[terminalMac] = b.mac where sessionID != '' and CONVERT (date, timestamp) between '"+req.param('start')+"' and '"+req.param('end')+"' and [os_name] = '"+req.param('os')+"'", function(err, rowCount){

...
var deviceList = function(req,res,next){
  ...
  pool.acquire(function(err, connection){
    ...
    var request = new Request(" if ('"+req.param('uid')+"') in (select [UID]  FROM [AgileControllerDB].[dbo].[UMS_UserStaging]) BEGIN UPDATE [AgileControllerDB].[dbo].[UMS_UserStaging] SET [PERSON_NID] = '"+req.param('ssn')+"',[PERSON_TH_NAME] = N'"+req.param('fn')+"',[PERSON_TH_SURNAME] = N'"+req.param('ln')+"',[LINE_POSITION_NAME] = N'"+req.param('pos')+"',[LEVEL] = N'"+req.param('lv')+"',[AREA] = N'"+req.param('area')+"' WHERE [UID] = '"+req.param('uid')+"'; end else begin INSERT INTO [AgileControllerDB].[dbo].[UMS_UserStaging] ([UID],[PERSON_NID],[PERSON_TH_NAME],[PERSON_TH_SURNAME],[LINE_POSITION_NAME],[LEVEL],[AREA])VALUES('"+req.param('uid')+"','"+req.param('ssn')+"','"+req.param('fn')+"','"+req.param('ln')+"','"+req.param('pos')+"','"+req.param('lv')+"','"+req.param('area')+"'); end;", function(err, rowCount){

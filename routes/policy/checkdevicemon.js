...
var deviceList = function(req,res,next){
  ...
  pool.acquire(function(err, connection){
    ...
    var request = new Request("select accountName,userName,orgName,terminalMac,timestamp from [AgileControllerDB].[dbo].[TSM_E_RadiusLoginOrLogoutLog] where CONVERT (date,timestamp) = CONVERT (date,CURRENT_TIMESTAMP ) and (CONVERT (time,timestamp) < (select CONVERT (time,c_value ) from UMS_Config where c_name = 'accesstimefrom') or CONVERT (time,timestamp) > (select CONVERT (time,c_value ) from UMS_Config where c_name = 'accesstimeto'))", function(err, rowCount){
